...
class Photometry():
    def __init__(self):
        ...
        self.saturation_limit = 500000 # pixel limit for instrument. HAWKI is 45000 # https://www.eso.org/sci/facilities/paranal/instruments/hawki/inst.html
...
imgpath = '/Volumes/PortableSSD/real_coordinates'
image_files = []
...
for filename in image_files:
    ...
    # Plot
    data = phot.get_data(path)
    data -= np.median(data.flatten()) # added this
    segment_map = phot.segment_map(path)
    segment_map -= np.median(segment_map.flatten()) # added this (which doesn't work)
    segm_deblend = phot.segm_deblend(path) #change this
    ...
    SimpleInfo_df = pd.concat([SimpleInfo_df, df_red], ignore_index=True)
